...
class OrderHistoryDetailRiderActivity : AppCompatActivity() {

    private lateinit var binding: ActivityOrderHistoryDetailRiderBinding
    val orderHistoryDetailList = arrayListOf<OrderHistoryDetail>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityOrderHistoryDetailRiderBinding.inflate(layoutInflater)
        setContentView(binding.root)
        val data = intent.extras
        val riderData: infoUserParcel? = data?.getParcelable("riderData")

        binding.btnBackToOrderHistoryRider.setOnClickListener {
            val orderHistoryRider = Intent(this@OrderHistoryDetailRiderActivity, OrderHistoryRiderActivity::class.java)
            orderHistoryRider.putExtra("riderData", riderData)
            startActivity(orderHistoryRider)
        }

        binding.orderList.adapter = ProductOrderHistoryRiderAdapter(this.orderHistoryDetailList, applicationContext)
    ...
    fun callReceiptData(){
        ...
        api.retrieveOrderHistoryDetail(
            ...
            override fun onResponse(call: Call<List<OrderHistoryDetail>>, response: Response<List<OrderHistoryDetail>>) {
                ...
                binding.orderList.adapter = ProductOrderHistoryRiderAdapter(orderHistoryDetailList, applicationContext)
            }
